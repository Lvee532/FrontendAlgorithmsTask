...
function checkDataType(data) {
    ...
    if (typeof (data) == "object") {
        if (Array.isArray(data) == true) {          //Checks that (data) of type 'object' is an array
            return "array";
        } else {
            return "object"
        }
        ...
        if (typeof (data) == "string") {
            if ((data != "") && !isNaN(data)) {     //Checks that (data) of type 'string' is not empty ("")
...
function convertFahrToCelius(TFah) {
    ...
    if (checkDataType(TFah) == "number") {
        ...
        console.log(`${JSON.stringify(TFah)} is not a valid number but a/an ${checkDataType(TFah)}`)
        return `${JSON.stringify(TFah)} is not a valid number but a/an ${checkDataType(TFah)}`
    }
}
